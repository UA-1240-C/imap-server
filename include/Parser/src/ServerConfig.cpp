...
void Config::ParseCommunicationSettings(const JSON &root)
{
    if (root.get_object_value().count("communicationsettings"))
    {
        ...
        communication_settings.blocking =
            GetValueOrDefault(comm_settings_obj, "blocking", communication_settings.blocking);
        communication_settings.socket_timeout =
            GetValueOrDefault(comm_settings_obj, "socket_timeout", communication_settings.socket_timeout);
    }
    else
    {
...
void Config::ParseThreadPoolConfig(const JSON &root)
{
    if (root.get_object_value().count("threadpool"))
    {
        ...
        thread_pool.max_working_threads =
            GetValueOrDefault(thread_pool_obj, "maxworkingthreads", thread_pool.max_working_threads);
    }
    else
    {
...
template <typename T>
T Config::GetValueOrDefault(const std::unordered_map<std::string, JSON> &obj, const std::string &key,
